...
def read_image_for_eval(path: str) -> np.ndarray:
    """
    評価パイプライン専用の画像読み込み関数。
    学習時と完全に同じロジック(preprocessing.py)を通す。
    """
    # return_dual=False を指定して、評価用の8-bit NumPy配列を取得
    img = read_and_preprocess_thermal(path, return_dual=False)
    if img is None:
        print(f"[Warning] Failed to load or preprocess image: {path}")
    return img
